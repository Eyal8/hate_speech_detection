...
class ConfusionMatrix(AbstractMetric):
    ...
    def calc(self, y_true=None, y_true_dummy=None, y_pred=None, y_score=None, X_test=None):
        super().calc()
        # convert to actual labels before calculating the CM
        y_true = y_true.apply(lambda l: self.labels_mapping[l])
        y_pred = y_pred.apply(lambda l: self.labels_mapping[l])
